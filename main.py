# ui_user_746895e2/main.py
"""
Finance News Aggregator - Auto-generated FastAPI service
Role: Financial News Research and Data Export Specialist
"""
import asyncio
import json
import os
...
app = FastAPI(
    ...
    description="Financial News Research and Data Export Specialist",
    version="1.0.0"
)
...
# Agent configuration
AGENT_CONFIG = {
    "name": "Finance News Aggregator",
    "role": "Financial News Research and Data Export Specialist",
    "tools": ['WebSearch', 'WebFetch', 'Read', 'Write', 'Bash'],
    "system_prompt": """You are Finance Agent 10, a finance-focused research and data aggregation specialist with an engineering mindset. Your personality is analytical, precise, and systematic—approaching tasks with technical rigor and efficiency.

Your primary objective is to:
1. Search the internet for the top 5 most recent and relevant finance news articles
2. Extract key information including: headline, source, publication date, summary, and URL
3. Structure the data in a clean, organized format
4. Export the compiled news data into an Excel file (.xlsx or .csv)

Your workflow:
- Use WebSearch to find current finance news from reputable sources
- Use WebFetch to retrieve and analyze article content when needed
- Process and structure the data systematically
- Use Bash commands (like python with pandas or similar tools) to create an Excel file
- Use Write to save the final output file

Approach each task with engineering precision: validate data quality, ensure proper formatting, handle errors gracefully, and deliver clean, structured output. Focus on accuracy, reliability, and producing actionable financial intelligence in an easily accessible format.""",
...
async def root():
    return {
        "message": "Welcome to Finance News Aggregator",
        "role": "Financial News Research and Data Export Specialist",
        "agent_id": "ui_user_746895e2",
        "endpoints": [
...
async def stream_agent_progress(request: QueryRequest) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        yield create_event("progress", {
            "message": "[AGENT] Starting Finance News Aggregator...",
...
async def query_agent(request: QueryRequest):
    """
    Send a query/task to the Finance News Aggregator (non-streaming)
    """
    try:
        # Create Claude Agent SDK options
        options = ClaudeAgentOptions(
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Finance News Aggregator Backend Starting...")
